...
object JDBSTest {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      ...
      .master("local")
      .getOrCreate()
    //读取hadoop中的csv文件，并写到mysql数据库中
    val optMap = Map("header"->"true", "infoSechma"->"true")
    val personDF =  spark.read.options(optMap).csv(s"${BASIC_PATH}/spark/dataset/jdbc_demo_data.csv")
    personDF.show()
    val properties = new Properties()
    properties.put("user", "root")
    properties.put("passwd", "12345678")
    //从数据到mysql
    personDF.write.mode(SaveMode.Overwrite).jdbc(JDBC_URL, "person",properties)
    //从mysql读取数据
    spark.read.jdbc(JDBC_URL, "person",properties)
    ...
    personDF.write.mode(SaveMode.Overwrite).options(writeOpts).jdbc(JDBC_URL, "person", properties)
    //2: 设置表的schema
    ...
    val jdbcpersonDF = spark.read.options(readOpts).jdbc(JDBC_URL, "person", properties)
    jdbcpersonDF.rdd.partitions.size
    jdbcpersonDF.rdd.glom().collect()
    jdbcpersonDF.show()

    //api的方式
    spark.read.jdbc(JDBC_URL, "person", "id", 3, 8, 3, properties).show()

    //参数predicates: Array[String],用于决定每一个分区对应的where子句，分区数就是数组predicates的大小
    val conditionpersonDF = spark.read.jdbc(JDBC_URL,
      "person", Array("id > 2 and id < 5", "id >= 5 and id < 8"), properties)
    conditionpersonDF.rdd.partitions.size
    conditionpersonDF.rdd.glom().collect()
    conditionpersonDF.show()
