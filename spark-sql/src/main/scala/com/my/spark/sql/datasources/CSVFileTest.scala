...
object CSVFileTest {
  def main(args: Array[String]): Unit = {
    ...
    //将json文件数据转化成csv文件数据
    df.write.mode(SaveMode.Overwrite).csv(s"${BASIC_PATH}/csv")

    val csvDF = spark.read.csv(s"${BASIC_PATH}/csv").toDF("age", "name")
    csvDF.show()
    ...
    ds.write.mode(SaveMode.Overwrite).option("sep", "|").csv(s"${BASIC_PATH}/delimiter")

    //2: header(默认是false) 表示是否将csv文件中的第一行作为schema(读写参数)
    spark.read.csv(s"${BASIC_PATH}/cars.csv").show()
    /*
    +----+-----+-----+--------------------+-----+
    | _c0|  _c1|  _c2|                 _c3|  _c4|
    +----+-----+-----+--------------------+-----+
    ...
    +----+-----+-----+--------------------+-----+
     */
    val headerDF = spark.read.option("header", true).csv(s"${BASIC_PATH}/cars.csv")
    headerDF.printSchema()
    headerDF.write.mode(SaveMode.Overwrite).option("header", true).csv(s"${BASIC_PATH}/headerDF")
    /*
    root
    ...
    val inferSchemaDF =
      spark.read.option("header", true).option("inferSchema", true).csv(s"${BASIC_PATH}/cars.csv")
    ...
    spark.read.option("header", "true").option("encoding", "iso-8859-1").option("sep", "þ").csv(s"${BASIC_PATH}/cars_iso-8859-1.csv").show()
    /*
    +----+-----+-----+--------------------+-----+
    |year| make|model|             comment|blank|
    +----+-----+-----+--------------------+-----+
    ...
    spark.read.options(optMap).csv(s"${BASIC_PATH}/comments.csv").show()
    /*
    +---+---+---+---+----+-------------------+
    ...
    val numbers = spark.read.format("csv").schema(StructType(List(
        ...
        "nanValue" -> "NAN",
        "negativeInf" -> "-INF",
        "positiveInf" -> "INF")).load(s"${BASIC_PATH}/numbers.csv")
    numbers.show()
    /*
    +----+--------+---------+---------------+
    ...
    inferSchemaDF.write.mode(SaveMode.Overwrite).option("compression", "gzip").csv(s"${BASIC_PATH}/compression")
     ...
     */
    date1.write.mode(SaveMode.Overwrite).option("dateFormat", "yyyy-MM-dd").csv(s"${BASIC_PATH}/dateFormat")
    spark.read.csv(s"${BASIC_PATH}/dateFormat").show()
     ...
     */
    time.write.mode(SaveMode.Overwrite).option("timestampFormat", "yyyy-MM-dd HH:mm").csv(s"${BASIC_PATH}/timestampFormat")
    spark.read.csv(s"${BASIC_PATH}/timestampFormat").show()
