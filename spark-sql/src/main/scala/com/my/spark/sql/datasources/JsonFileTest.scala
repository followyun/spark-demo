...
object JsonFileTest {
  def main(args: Array[String]): Unit = {
    ...
    val sessionDf = spark.read.parquet(s"${BASIC_PATH}/trackerSession")
    sessionDf.show()

    sessionDf.write.json(s"${BASIC_PATH}/json")

    //读取json文件数据
    val jsonDF = spark.read.json(s"${BASIC_PATH}/json")
    jsonDF.show()
    ...
    spark.read.option("mode", "PERMISSIVE").json(Seq(
      """{'name':'Yin',"address":{"is_old":true,"area":3000}}""",
    ...
    spark.read.option("mode", "DROPMALFORMED").json(Seq(
      """{'name':'Yin',"address":{"is_old":true,"area":3000}}""",
      """{'name':'Yin',"address":{"is_old":true,"area":\3000}}""").toDS()).show()
    /*
    +-----------+----+
    ...
    spark.read.option("mode", "FAILFAST").json(Seq(
      """{'name':'Yin',"address":{"is_old":true,"area":3000}}""",
      """{'name':'Yin',"address":{"is_old":true,"area":\3000}}""").toDS()).show()
    ...
    val primitiveFieldAndType: Dataset[String] = spark.createDataset(spark.sparkContext.parallelize(
      """{"string":"this is a simple string.",
        ...
        """{"string":"this is a simple string.",
          ...
          |          "null":null
          |      }""" :: Nil))(Encoders.STRING)
    primitiveFieldAndType.toDF("value").write.mode(SaveMode.Overwrite).option("compression", "GzIp").text(s"${BASIC_PATH}/primitiveFieldAndType")

    val multiLineDF = spark.read.option("multiLine", false).json(s"${BASIC_PATH}/primitiveFieldAndType")
    multiLineDF.show()
